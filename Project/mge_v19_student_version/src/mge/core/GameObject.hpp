...
{
	public:
		GameObject(const std::string& pName = "", const glm::vec3& pPosition = glm::vec3( 0.0f, 0.0f, 0.0f ));
		virtual ~GameObject();
		...
		void setLocalScale (glm::vec3 pScale);
		void setLocalRotation (glm::mat3 pRotation);
		glm::vec3 getLocalPosition() const;
		glm::vec3 getLocalScale() const;
		glm::mat3 getLocalRotation() const;

		void setWorldPosition(glm::vec3 pPosition);
		void setWorldScale(glm::vec3 pScale);
		void setWorldRotation(glm::mat3 pRotation);
		...
		void translateLocal(glm::vec3 pTranslation);
		void rotateLocal(float pAngle, glm::vec3 pAxis);
		void scaleLocal(glm::vec3 pScale);
		...
		template<class T, typename = typename std::enable_if_t<std::is_base_of_v<AbstractBehaviour, T>>>
		void addBehaviour(T* pBehaviour);
	...
	protected:
        ...
        Mesh* _mesh;
		std::unordered_map<std::type_index, AbstractBehaviour*> _behaviours;
...
inline void GameObject::addBehaviour(T * pBehaviour)
{
	_behaviours[typeid(T)] = pBehaviour;
	pBehaviour->setOwner(this);
}

template<class T, typename>
inline T * GameObject::getBehaviour() const
{
	return dynamic_cast<T*>(_behaviours[typeid(T)]);
}
