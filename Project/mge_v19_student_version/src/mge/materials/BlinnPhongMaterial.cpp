...
GLint BlinnPhongMaterial::_uViewProjectionMatrix = 0;
GLint BlinnPhongMaterial::_uModelMatrix = 0;
GLint BlinnPhongMaterial::_uAlbedo = 0;
GLint BlinnPhongMaterial::_uNormal = 0;
GLint BlinnPhongMaterial::_uMetallic = 0;
GLint BlinnPhongMaterial::_uRoughness = 0;
GLint BlinnPhongMaterial::_uAO = 0;
GLint BlinnPhongMaterial::_uHeight = 0;
GLuint BlinnPhongMaterial::_uboLights = 0;
GLint BlinnPhongMaterial::_uLightCount = 0;
GLint BlinnPhongMaterial::_uCameraPosition = 0;
GLint BlinnPhongMaterial::_uHeightScale = 0;

GLint BlinnPhongMaterial::_aVertex = 0;
GLint BlinnPhongMaterial::_aNormal = 0;
GLint BlinnPhongMaterial::_aUV = 0;
GLint BlinnPhongMaterial::_aTangent = 0;

BlinnPhongMaterial::BlinnPhongMaterial(std::string name, Texture* pAlbedoMap, Texture* pNormalMap, Texture* pMetalMap, Texture* pRoughness, Texture* pAmbientOcclusion, Texture* pHeight, float pHeightScale) :
	Material<BlinnPhongMaterial>(name),
...
void BlinnPhongMaterial::LazyInitializeShader() {
	if (!_shader) {
		_shader = new ShaderProgram();
		_shader->addShader(GL_VERTEX_SHADER, config::MGE_SHADER_PATH + "littextureBlinnPhong.vert");
		_shader->addShader(GL_FRAGMENT_SHADER, config::MGE_SHADER_PATH + "littextureBlinnPhong.frag");
		_shader->finalize();
		...
		Material<BlinnPhongMaterial>::LazyInitializeShader();
...
void BlinnPhongMaterial::Bind(World * pWorld, const glm::mat4 & pViewMatrix, const glm::mat4 & pProjectionMatrix, Mesh * mesh)
{
...
void BlinnPhongMaterial::Release(Mesh * mesh)
{
	mesh->Unbind(_aVertex, _aNormal, _aUV, _aTangent);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
	_shader->clearUsage();
...
void BlinnPhongMaterial::setDiffuseTexture(Texture* pAlbedoMap) {
	albedo = pAlbedoMap;
}

void BlinnPhongMaterial::render(World* pWorld, Mesh* pMesh, const glm::mat4& pModelMatrix, const glm::mat4& pViewMatrix, const glm::mat4& pProjectionMatrix) {
	if (!albedo)
	{
		std::cout << "Albedo map for material " << name << " is missing!!!" << std::endl;
		albedo = Texture::load(config::MGE_TEXTURE_PATH + "Default/default-albedo.png");
